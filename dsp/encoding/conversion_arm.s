...
TEXT ·u8ToI16Asm(SB), NOSPLIT, $0
	MOVW src+12(FP), R1
	MOVW src_len+16(FP), R2
	MOVW dst+0(FP), R3
	MOVW dst_len+4(FP), R4
...
TEXT ·u8ToI16LEAsm(SB), NOSPLIT, $0
	MOVW dst_len+4(FP), R4
	MOVW R4>>1, R4
	MOVW R4, dst_len+4(FP)
	B    ·u8ToI16Asm(SB)
...
u8tof32_tail_loop:
	...
	CMP   R2, R1
	BLT   u8tof32_tail_loop

u8tof32_done:
	RET

// TODO
TEXT ·i8ToF32Asm(SB), NOSPLIT, $0
	B ·i8ToF32(SB)

TEXT ·U8ToC64(SB), NOSPLIT, $0
	MOVW src_len+16(FP), R2
	AND  $(~1), R2
	MOVW R2, src_len+16(FP)
	MOVW dst_len+4(FP), R0
	MOVW R0<<1, R0
	MOVW R0, dst_len+4(FP)
	B    ·U8ToF32(SB)

// VCVT.S32.F32 saturates to the int32 range and converts NaN to zero, but the
// stores below keep only the low halfword of each result, so every lane still
// has to be saturated to int16 or an overdriven sample folds to the opposite
// rail. SSAT does that in one instruction; it is ARMv6, which this file already
// requires because it uses VFP unconditionally, and the Go assembler does not
// know the mnemonic so it is encoded with WORD like the VFP instructions here.
//
// There is no VFP-vector path here any more. This function used to have one, but
// the only vector-mode instruction in it was the multiply by the scale
// argument -- VCVT is scalar even in vector mode -- so once the scale went away
// the vector-length dance around the loop cost four instructions per call and
// bought nothing, and the useVector flag went with it. dsp/f32 still keeps one,
// for math32_arm.s there, where there is real vector work to do.
TEXT ·f32ToI16Asm(SB), NOSPLIT, $0
	MOVW src+12(FP), R1
	MOVW src_len+16(FP), R2
	MOVW dst+0(FP), R3
	MOVW dst_len+4(FP), R0
...
f32toI16_loop:
	...
	WORD $0xe6af0010 // ssat r0, #16, r0
	WORD $0xe6af4014 // ssat r4, #16, r4
	MOVH R0, 0(R3)
	MOVH R4, 2(R3)
	WORD $0xec5b8a12 // vmov r8, r11, s4, s5
	WORD $0xe6af8018 // ssat r8, #16, r8
	WORD $0xe6afb01b // ssat r11, #16, r11
	MOVH R8, 4(R3)
	MOVH R11, 6(R3)
	ADD  $8, R3
...
TEXT ·f32ToI16LEAsm(SB), NOSPLIT, $0
	MOVW dst_len+4(FP), R0
	MOVW R0>>1, R0
	MOVW R0, dst_len+4(FP)
	B    ·f32ToI16Asm(SB)

TEXT ·i16ToI16LEAsm(SB), NOSPLIT, $0
	B ·i16ToI16LE(SB)

TEXT ·i16LEToF64Asm(SB), NOSPLIT, $0
	B ·i16LEToF64(SB)

TEXT ·i16LEToF32Asm(SB), NOSPLIT, $0
	B ·i16LEToF32(SB)
